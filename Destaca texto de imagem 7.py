...
"""
Este programa realiza o **reconhecimento de palavras ou frases específicas em imagens** e permite destacá-las, apagá-las e numerá-las automaticamente. Ele utiliza OCR (Reconhecimento Óptico de Caracteres) via `pytesseract` e é ideal para gerar atividades educativas, provas com lacunas, gabaritos anotados e materiais com foco em revisão de conteúdo.

FUNCIONALIDADES PRINCIPAIS:

1. **Entrada de Caminho da Pasta**:
   O usuário informa (via terminal ou argumento) o caminho de uma pasta contendo imagens. O programa valida se o caminho existe e se contém imagens suportadas (JPG, PNG, BMP, TIFF).

2. **Inserção de Palavras-Chave**:
   O usuário digita as palavras ou frases que deseja localizar e destacar nas imagens. A busca é feita ignorando diferenças entre maiúsculas e minúsculas.

3. **Modos de Execução**:
   - **Modo Simples**: Destaca ou apaga as palavras-chave nas imagens, salvando a versão final em uma pasta "Destacado-NomeDaPasta".
   - **Modo Duplo ("Tudo")**: Cria duas versões para cada imagem:
     - **Gabarito**: com as palavras visíveis destacadas.
     - **Prova**: com as palavras apagadas (e opcionalmente numeradas).

4. **Numeração Sequencial**:
   Se ativada, a numeração (1, 2, 3...) aparece no local onde as palavras foram apagadas, ideal para exercícios com respostas numeradas.

5. **Exclusão Automática de Imagens Não Relevantes**:
   Caso o usuário deseje, imagens que não contenham nenhuma palavra-chave são deletadas automaticamente após o processamento.

6. **OCR com pytesseract**:
   - O texto é lido imagem por imagem, bloco por bloco.
   - Palavras/frases são detectadas e cercadas por retângulos vermelhos.
   - Palavras apagadas são cobertas por uma tarja branca.
   - Fontes são utilizadas para desenhar os números (quando ativado).

7. **Relatórios e Logging**:
   Todos os eventos são registrados em tempo real:
   - Log detalhado no terminal e no arquivo `processamento_imagens.log`.
   - Relatório final com número de imagens processadas, palavras destacadas, erros, exclusões, e pastas de saída.

REQUISITOS:
- Python 3
- Bibliotecas: `pytesseract`, `Pillow`, `tqdm`, `colorama`
- Tesseract OCR instalado e configurado corretamente no Windows

USO RECOMENDADO:
- Professores criando provas e gabaritos.
- Estudantes revisando conteúdo visual.
- Anonimização de imagens com texto.
- Aplicações que exigem ocultar termos visuais automaticamente.
